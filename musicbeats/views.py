...
from musicbeats.models import Song, Watchlater, History, Channel, Album
...
def search(request):
    ...
    qs = albums.filter(Album_name__icontains =query)
    qs = song.filter(name__icontains = query)
    return render(request, 'musicbeats/search.htm', {"songs": qs, "album": qs, "query":query})
...
def albums(request):
    albums = Album.objects.all()
    return render(request, "musicbeats/album.htm", {'album': albums})

def albumpost(request, id):
    albums = Album.objects.filter(Album_id=id).first()
    return render(request, "musicbeats/albumpost.htm", {'album': albums})

def genre_list(request):
    genre_present= Song.objects.values('tags').distinct()
    return render(request, "musicbeats/genre.htm", {"genre_list": genre_present,})

def tags(request, tags_name):
    tags = Song.objects.filter(tags = tags_name)
    print(tags)
    return render(request, "musicbeats/genrepost.htm", {"tags" : tags,})
